...
// Full-day NYSE/NASDAQ market holidays, keyed by ET calendar date
// (YYYY-MM-DD) -- mirrors the identical, source-checked list in
// Tra/trade-verdict's server.js isMarketOpen(). isMarketClosed() itself is
// deliberately tier-owned, not in shared/rolodex.ts, so this list is kept
// alongside it (hand-duplicated across every tier's own copy) rather than
// shared -- needs a manual update for 2028+, source-checked (Good Friday
// especially) before extending, same as the backend copy.
var MARKET_HOLIDAYS = new Set([
  '2026-01-01', '2026-01-19', '2026-02-16', '2026-04-03', '2026-05-25',
  '2026-06-19', '2026-07-03', '2026-09-07', '2026-11-26', '2026-12-25',
  '2027-01-01', '2027-01-18', '2027-02-15', '2027-03-26', '2027-05-31',
  '2027-06-18', '2027-07-05', '2027-09-06', '2027-11-25', '2027-12-24',
]);
var MARKET_EARLY_CLOSE_DAYS = new Set(['2026-11-27', '2026-12-24', '2027-11-26']);
...
// refresh paths this app already has -- fetchMarket() (no client-side
// cache to bypass on Free, unlike Starter/Pro's force-refresh flag) and
// fetchTickerData(sym, true) -- no new fetch mechanism. Free has no
// card/overflow split (its whole watchlist IS the pill strip, capped at
// 3), so the repaint scope here is simply every ticker, not a windowed
...
async function runAgitatorCheck(): Promise<void> {
  ...
  try {
    ...
    if (data.commodity) {
      ...
      var cmRelatedHTML = '<div class="track-log-title" style="margin-top:10px">RELATED</div>'
        + ((cm.related && cm.related.length)
    ...
    if (!data.resolved) {
      ...
      if (topical) {
        ...
        // there's genuinely no tradable correlation at all. When the
        // topical fallback DID validate a real company mentioned in the
        // article (topical.companies, already the same real, hyperlinked
        // tickers the RELATED list below shows), the first/most relevant
        // one is what's actually being scored -- show it, don't hide it
        // behind dashes.
...
// tutorial state), adapted to Free's own smallest card set (pulse,
// agitator, io, glossary -- no Dial, no Scorecard) and its own tier-scoped
// storage flag so a Free user who later upgrades to Starter/Pro still sees
// that tier's own walkthrough once, independent of this one.
