import { getAccuracyLog, replaceLog } from './track-record.js?v=17';
...
// visitors.
interface TrackRecordSyncConfig {
  API_URL: string;
  authH: () => Record<string, string>;
  addSecret: (url: string) => string;
}

let cfg: TrackRecordSyncConfig | null = null;
let pulling = false; // suppresses the push a pull's own replaceLog() would otherwise trigger

export function initTrackRecordSync(config: TrackRecordSyncConfig): void { cfg = config; }
...
export async function pullTrackRecordFromServer(): Promise<void> {
  if(!cfg)return;
  var data: { entries?: ReturnType<typeof getAccuracyLog> } | null = null;
...
// per tap.
export function schedulePushTrackRecord(): void {
  if(!cfg||pulling)return;
  if(pushTimer)clearTimeout(pushTimer);
  pushTimer=setTimeout(function(){pushTrackRecordToServer(false);},1200);
}

async function pushTrackRecordToServer(seed: boolean): Promise<void> {
  if(!cfg)return;
  try{
