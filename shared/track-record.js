...
function saveLog(log) { localStorage.setItem(LOG_KEY, JSON.stringify(log)); if (saveHook)
    saveHook(); }
...
export function clearLog() { if (!confirm('Clear all logged trades?'))
    return; saveLog([]); renderTrackRecord(); }
...
export function renderTrackRecord() {
    ...
    log.forEach(function (e) { var v = e.verdict || 'UP'; if (!byType[v])
        byType[v] = { c: 0, t: 0 }; byType[v].t++; if (e.correct)
        byType[v].c++; });
    var typeRate = function (v) { return byType[v].t ? Math.round((byType[v].c / byType[v].t) * 100) + '%' : '&mdash;'; };
    var byTicker = {};
    log.forEach(function (e) { if (!byTicker[e.ticker])
        byTicker[e.ticker] = { c: 0, t: 0 }; byTicker[e.ticker].t++; if (e.correct)
        byTicker[e.ticker].c++; });
    ...
    for (var i = log.length - 1; i >= 0; i--) {
        if (streakType === null)
            streakType = log[i].correct;
        if (log[i].correct === streakType)
            streak++;
        else
            break;
    }
