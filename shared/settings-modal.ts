import { TIMEZONES, LINK_SITES, getTzPref, setTzPref, getLinkSitePref, setLinkSitePref, getCustomTemplate, setCustomTemplate, getCustomMarketTemplate, setCustomMarketTemplate, buildTemplateFromExample, detectTickerInUrl } from './prefs.js?v=11';
...
function ensureModal(): HTMLElement {
  ...
  (document.getElementById('settings-close-btn') as HTMLButtonElement).addEventListener('click', closeSettingsModal);
  var tzSel = document.getElementById('settings-tz') as HTMLSelectElement;
  ...
  var linkSel = document.getElementById('settings-link-site') as HTMLSelectElement;
  Object.keys(LINK_SITES).forEach(function(k){
    var o = document.createElement('option');
    o.value = k; o.textContent = LINK_SITES[k].label;
    linkSel.appendChild(o);
  });
  var customWrap = document.getElementById('settings-custom-wrap') as HTMLElement;
  var exUrl = document.getElementById('settings-custom-ex-url') as HTMLInputElement;
  var customStatus = document.getElementById('settings-custom-status') as HTMLElement;
  var marketUrl = document.getElementById('settings-custom-market-url') as HTMLInputElement;
  var marketStatus = document.getElementById('settings-custom-market-status') as HTMLElement;
...
export function openSettingsModal(): void {
  var el = ensureModal();
  (document.getElementById('settings-tz') as HTMLSelectElement).value = getTzPref();
  (document.getElementById('settings-link-site') as HTMLSelectElement).value = getLinkSitePref();

  (document.getElementById('settings-custom-ex-url') as HTMLInputElement).value = '';
  var saved = getCustomTemplate();
  var statusEl = document.getElementById('settings-custom-status') as HTMLElement;
  statusEl.textContent = saved ? 'Currently: ' + saved : 'No link saved yet — using Yahoo Finance.';
  statusEl.style.color = 'var(--dim)';

  (document.getElementById('settings-custom-market-url') as HTMLInputElement).value = '';
  var savedMarket = getCustomMarketTemplate();
  var marketStatusEl = document.getElementById('settings-custom-market-status') as HTMLElement;
  ...
  (document.getElementById('settings-custom-wrap') as HTMLElement).style.display = getLinkSitePref() === 'custom' ? 'block' : 'none';
