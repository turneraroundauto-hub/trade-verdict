...
async function renderScorecardCard(): Promise<void> {
  ...
  try {
    ...
    // a first build had drifted from (direct screenshot feedback): pooled
    // stats render as two stat tiles under a "Pooled · N graded" badge,
    // not a stack of full-width label/value rows.
    ...
    // Replaced by the tiles above -- directional accuracy answers "was
    // the call right"; avg return answers "would following it at the
...
async function runAgitatorCheck(): Promise<void> {
  ...
  try {
    ...
    // Past Reactions (Fix 4, Sep 1 2026) reads the real value now instead
    // of a permanently hardcoded "not tracked yet" -- a real oversight in
    // the same pass that activated it server-side; agitatorFactorRow's own
    // null-check already renders "n/a" when a ticker has too little graded
    // history.
    var f = data.factors;
    var factorsHTML = f
      ? '<div class="track-log-title" style="margin-top:10px">SIGNALS</div>'
