...
function relatedRowHTML(c) {
  ...
  return '<div class="compact-row-wrap" data-ticker="' + t + '"><div class="compact-row"><div class="compact-row-main"><div class="compact-row-top"><span class="compact-ticker" style="color:' + color + '"><a class="ticker-a" href="' + tickerHref2(t) + '" target="_blank">' + t + '</a></span><span class="compact-price" style="color:' + color + '">' + (c.price ? "$" + c.price : "&mdash;") + '</span><span class="compact-pct" style="color:' + color + '">' + (c.change || "&mdash;") + '</span></div><div class="compact-news"' + (hasNews ? "" : ' style="display:none"') + ">" + (hasNews ? wrapHeadlineLinks(t, autoLinkGlossaryTerms(highlightContextMatches(c.news.headline, ctxVal))) : "") + "</div></div>" + addTickerBtnHTML(t) + "</div></div>";
}
...
async function runAgitatorCheck() {
  ...
  try {
    ...
    if (data.commodity) {
      ...
      var cmGaugeHTML = '<div class="trigger-row" style="margin-top:8px"><span class="trigger-lbl-wrap"><span class="trigger-lbl"><a href="' + tickerHref2(cm.proxyTicker) + '" target="_blank">' + cm.proxyTicker + '</a></span></span><span class="trigger-val-wrap"><span class="trigger-val" style="color:' + cmGaugeColor + '">' + (cmComp ? cmComp.level : "N/A") + '</span><button type="button" class="help-btn" data-help="agitator-score" aria-label="What is this?">?</button></span><span class="trigger-sub">' + (cmComp ? Math.round(cmComp.score / 10) + "/10" : "no data") + "</span></div>";
      var cmf = cm.factors;
    ...
    if (!data.resolved) {
      ...
      if (topical) {
        ...
        var tTickerLabel = tPrimary ? '<a href="' + tickerHref2(tPrimary.symbol) + '" target="_blank">' + tPrimary.symbol + "</a>" : "---";
